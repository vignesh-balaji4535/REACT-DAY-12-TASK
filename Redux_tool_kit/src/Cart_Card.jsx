...
import { removeCartData, setCartCount} from './Redux_Slice';
...
const CartCount=useSelector((state)=>state.Shopping.CartCount);
const CartData=useSelector((state)=>state.Shopping.CartData)


const remove2=()=>{
  if(CartCount>0){
    dispatch(setCartCount("-"))}
  let deleted =CartData.filter((item,ind)=>ind!==index);
    alert("DO YOU WANT TO REMOVE THIS ITEM")
    dispatch(removeCartData(deleted))
  }
    ...
    <div>
        <div className="row border rounded-3 border-info border-2 my-5">
            ...
            <div className="col-lg-3 gx-2 h-25  p-3">
              ...
              <p className='px-4 border rounded-2' style={{marginBottom:"0"}} >{count}</p>
                <div className='d-flex '>
                    <button className='btn p-1 m-0 w-50 'onClick={()=>{
                       count<stock&&  CartData.map((item,ind)=>{
                    if(ind==index){
                      setCount(count+1);
